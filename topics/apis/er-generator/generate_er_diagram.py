...
def document_type_entities(doc_type=None):
    with open(METADATA_FILE) as f:
        ar_structure = json.load(f)
...
def generate_diagram(document_type):
    er_markdown = ""

    if document_type != "":
        er_markdown = generate_er_markdown(document_type_entities(document_type))
    else:
        return None

    # Generate output file name
    document_filename = "_" + document_type if document_type != "" else ""
    
    er_markdown_filename = f"analytical_reporting{document_filename}.er"
    pdf_markdown_filename = f"analytical_reporting{document_filename}.pdf"

    # Store Entity Relationship markdown
    with open(er_markdown_filename, "w") as f:
        f.write(er_markdown)
    
    ## Draw from Entity Relationship markdown to PDF
    render_er(er_markdown_filename, pdf_markdown_filename)

    pdf_contents = None

    with open(pdf_markdown_filename, "rb") as f:
        pdf_contents = bytearray(f.read())

    os.remove(er_markdown_filename)
    os.remove(pdf_markdown_filename)
